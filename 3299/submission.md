...
```text
โจทย์ให้รับ L กับ Nแล้วหาว่าช่องสุดท้ายอยู่ในแถบเลขอะไร โดยแถบแต่ละแถบคือแนวทแยงติดกัน L เส้น ซึ่งมีจำนวนช่องเพิ่มขึ้นเรื่อยๆ
```
...
```text
เริ่มจากแถบที่ 1 แล้ววนเพิ่มทีละแถบ โดยแต่ละแถบนับจำนวนช่องที่ปลูกสะสมไว้ จนกว่าผลรวมสะสมจะถึงหรือเกิน N แล้วพิมพ์หมายเลขแถบนั้น
```
...
```text
วิธีสุดท้ายเหมือนแผนแรกครับคือไล่ทีละแถบแล้วนับช่องสะสมไปเรื่อยๆ จนถึง N ผมเลือกวิธีนี้เพราะ N ไม่เยอะ ลูปไม่กี่รอบก็จบเลยไม่ต้องคิดสูตรให้ยุ่งยาก
```
...
```text
ตรวจกรณี L > 1 ที่แถบมีจำนวนช่อง 6, 15, 24
```
...
```text
L และ N เล็กสุดตามที่โจทย์กำหนด ตรวจว่าลูปทำงานอย่างน้อยรอบเดียวและไม่คืน 0
```
...
```text
7 คือช่องแรกของแถบที่ 2 ใช้คู่กับ Test Case 4 เพื่อดูว่ารอยต่อระหว่างแถบถูกทั้งสองด้าน
```
