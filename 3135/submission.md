...
```text
โจทย์ต้องการให้รับค่าจำนวนคนทั้งหมด จำนวนตำแหน่งที่ขยับ และหมายเลขของคนที่ต้องการหา เอ้าพุทคือจำนวนคนที่ได้รับของขวัญ
```
...
```text
ผมจะรับค่าอินพุทก่อนแล้วกำหนดว่าคนหมายเลข1เป็นคนเริ่มและนับเป็นลำดับที่1 จากนั้นตรวจสอบว่าคนที่เริ่มตรวจสอบใช่tไหมถ้าใช่ให้แสดง1แล้วจบเลย หากไม่ก็ขยับไปเรื่อยๆจนกว่าจะใช่ และเราจะใช้moduloเพื่อให้เมื่อเดินเกินคนสุดท้าย สามารถวนกลับไปคนแรกได้เพิ่มจำนวนลำดับขึ้น 1 และเปลี่ยนคนปัจจุบันเป็นคนที่คำนวณได้ตรวจสอบว่าคนปัจจุบันเป็นคน t หรือไม่ถ้าใช่ให้หยุดถ้าไม่ก็ให้วนจนกว่าจะเจอtถึงวงหยึดทำงาน จากนั้นแสดงจำนวน
```
...
```text
วิธีสุดท้ายของผมเหมือนกับแผนแรกเลยโดยเริ่มจากคนที่ 1 แล้วขยับไปทีละ k ตำแหน่ง จากนั้นนับจำนวนครั้งที่เดินไปจนเจอคนหมายเลข t ถ้าวนกลับมาคนที่ 1 ก่อนเจอ t ก็หยุดการทำงาน
```
...
```text
ตรวจการขยับทีละ k ตำแหน่งและการนับลำดับตามปกติ
```
...
```text
ตรวจกรณีพิเศษที่ t = 1 ซึ่งไม่ต้องขยับเลย
```
...
```text
ตรวจว่าการทำให้ขยับเกินคนสุดท้ายแล้วสามารถวนกลับมาที่คนหมายเลข 1 ได้ถูกต้อง
```
