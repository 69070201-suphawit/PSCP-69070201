...
```text
โจทย์ต้องการให้เราหาวิธีออกปราสาทยังไงให้ทำลายกำแพงน้อยที่สุดโดยที่inputคือห้องที่เราอยู่(ห้องที่เราเริ่ม) และoutputที่โจทย์ต้องการคือจำนวนที่น้อยที่สุดที่เป็นไปได้ในการพังกำแพงเพื่อไปถึงจุดหมาย
```
...
```text
ทีแรกผมรับค่าinputมาก่อน จากนั้นจึงลองเขียนif elseในกรณีที่อยู่ห้อง1อยู่แล้วไม่ต้องทำลายกำแพง จากนั้นผมลองเขียนif else ในการทำชั้นต่อไปเรื่อยๆลองหาสูตรตาม serch engine ดู แต่ก็ยังคิดไม่ออกว่าควรทำอย่างไร 
```
...
```text
สุดท้ายผมใช้วิธีimport math มาก่อน จากนั้นรับinput แล้วเขียนฟังก์ชั่นif elseที่ถ้าอยู่ห้อง1อยู่แล้วไม่ต้องทำลายกำแพง จากนั้นนำมาแทนในสูตรในบรรทัดที่11โดยใช้ฟังชั่น math.ceilเพื่อปัดเศษ และ math.sqrtเพื่อใส่ สแควรูท ที่ต้องใส่สแควรูทเพราะ สังเกตว่าแต่ละชั้นห้องของมันจะมีค่าเป็นยกกำลัง2 เช่น ช้นที่2 มีห้องเป็นจำนวน2**2 จึงลองใช้สแควรูทดูดังนั้น ห้องหมายเลข n จะอยู่ในชั้น R = ceil(sqrt(n))ตามสูตร จากนั้นก็หาจำนวนห้องก่อนหน้า โดยนำRทีไ่ด้มา - 1 แล้วยกกำลังด้วย2 จากนั้นหาตำแหน่งห้องโดยใช้ P = n - จำนวนห้องก่อนหน้าที่หามา จากนั้นนำมาเข้าฟังก์ชั่น if else โดยที่ถ้า P หรือตำแหน่งห้องหารด้วย % 2 หากเป็นเลขคี่จะคำนวณด้วยสูตร wall = 2(2 * R) - 1 หากไม่ใช่จะใช้สูตร (2 * R) - 3 แทน
```
...
```text
อยากรู้ว่าถ้าใส่เป็นทศนิยมมันจะรันได้มั้ย
```
...
```text
ลองดูว่าถ้าหากใส่เลขที่เกินจำนวนพีรามิดมันจะได้มั้ย
```
...
```text
ลองดูว่าถ้าค่าติดลบมันจะยังรันได้มั้ย
```
...
```text
ช่วยแนะนำสูตรต่างๆ แนะนำให้ใช้import math และเสนอวิธีคิด logic
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนระบบโครงสร้างเอง เขียนฟังชั่นเองและเขียนโค้ดเอง
```
